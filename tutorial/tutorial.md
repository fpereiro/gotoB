...
User communication, if happening within the context of a single app, can be managed by the server itself.
...
Types of apps:
Native. All apps were like this before the web. Most apps in your phone are native.
Web apps: an application that runs in a browser.
What is a browser?
Start with examples of browsers. URL bar has an address that takes you to a page or an app.
Browsers can open both web pages and web apps.
What is a web app? It is an app that:
Runs on a browser: if you have a browser installed, there’s no install needed. You just go to the URL and open it.
Runs on any device that has a browser.
All of them are services: use a server. (Remember quadrant 2 of last video).
Why are they important?
Freedom: Native apps are controlled by either the device makers or the OS makers. You must be an approved developer to get your app distributed by them in the case of mobile. Web apps are powered by a set of standards that are not controlled by a single organization, but rather a large consensus.
Lightness: (we’re going to get some heat for saying this, but) web apps take less work than native apps to implement. [TODO: is this really worth it?]
Cross device: … [TODO]
In summary: avoid gatekeepers to reach the most people, and do it with less work, to boot.
